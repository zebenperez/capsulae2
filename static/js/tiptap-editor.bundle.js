var CapsulaeTiptapBundle = (() => {
  ...
  var CommandManager = class _CommandManager {
    ...
    static createFallbackCan() {
      const chain = _CommandManager.createFakeChain();
      const can = new Proxy(
        {
          chain: () => chain
        },
  ...
  var lift3 = (typeOrName, attributes = {}) => ({ state, dispatch }) => {
    const type = getNodeType(typeOrName, state.schema);
    const isActive22 = isNodeActive(state, type, attributes);
    if (!isActive22) {
      return false;
    }
  ...
  var toggleMark = (typeOrName, attributes = {}, options = {}) => ({ state, commands }) => {
    ...
    const type = getMarkType(typeOrName, state.schema);
    const isActive22 = isMarkActive(state, type, attributes);
    if (isActive22) {
...
.ProseMirror-focused .ProseMirror-gapcursor {
  ...
  function render(tag, attributes, hasMultipleChildren) {
    if (tag === "slot") {
      return 0;
    }
  ...
  var h = (tag, attributes) => render(tag, attributes, false);
...
3. "-" cannot repeat`);
  ...
  // static/js/tiptap-editor-src.js
  ...
  function isActive2(command2, editor) {
    switch (command2) {
      case "bold":
  ...
  function canRun(command2, editor) {
    switch (command2) {
      case "undo":
  ...
  function updateToolbar(instance) {
    ...
    wrapper.querySelectorAll(".tiptap-editor__button").forEach((button) => {
      const command2 = button.dataset.command;
      const active = isActive2(command2, editor);
      const enabled = canRun(command2, editor);
      button.classList.toggle("is-active", active);
      button.setAttribute("aria-pressed", active ? "true" : "false");
      button.disabled = !enabled;
    });
  }
  function runCommand(command2, editor) {
    switch (command2) {
      case "undo":
