...
from .models import FichaPrincipioActivo, PresentationsPrescriptionsAempsCache as AempsCache
...
def medication(request):
    return render(request, "medication/medication.html", {'items': [], 'update_params': get_update_params()})

#@group_required("admins",)
#def medication_list(request):
#    return render(request, "medication/medication-list.html", get_medication_context(request.user))

@group_required("admins",)
def medication_search(request):
    search_value = get_param(request.GET, "s-name")
    return render(request, "medication/medication-list.html", {'items': get_medication(search_value)})
...
def update_cache(request):
    ...
    return render(request, "medication/update-cache-box.html", {'update_params': get_update_params()})
